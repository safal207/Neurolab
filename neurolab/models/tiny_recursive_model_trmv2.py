"""
TinyRecursiveModelTRMv2 - Heartbeat-TRM v2 with attention mechanism.

Extends v1 with self-attention for temporal token relationships.
...
class TinyRecursiveModelTRMv2(nn.Module):
    """
    Heartbeat-TRM v2 - adds attention mechanism to competitive model.

    Improvements over v1:
    - Self-attention mechanism to model relationships across iterations
    - History tracking of latent states
    - Attention-based state refinement
    ...
    def forward(self, x, y_init, affect_vec=None, K=5):
        """
        Forward pass with attention-enhanced recursive refinement.
